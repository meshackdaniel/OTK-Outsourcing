...
export default function EmployerHelpCategoryPage() {
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 bg-gray-50/50">
            <h2 className="font-bold text-gray-900">Articles ({articles.length})</h2>
          </div>
          <div className="divide-y divide-gray-100">
            {articles.map((article, i) => (
              <Link key={i} href={`/employer/help/${categorySlug}/${generateSlug(article.title)}`} className="flex items-center justify-between p-6 hover:bg-gray-50 transition group">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center group-hover:bg-white group-hover:shadow-sm transition">
                    {article.type === "video" ? (
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 group-hover:text-[#222364] transition">{article.title}</h3>
